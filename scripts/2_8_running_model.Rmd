...
list.of.packages <- c("tidyverse", "lme4", "MuMIn", "rcompanion", "MASS", "generalhoslem", "mgcv", "beepr", "regclass")
...
allcrabs_dat <- crab_dat %>%
  dplyr::select(c(s.Year, Site, Sex, s.CW, Shell.Condition, Leg.Condition, Bitter, Blackmat, s.Latitude, s.Longitude, s.Jul.Day, s.Depth, s.Temp))
...
Here's the order of the impact of our variables, from most to least impactful. This'll determine the order in which we try to add these variables to our full model!
...
Based on this, we'll make a judgment call: We'll try to put Depth in before Lat/Long (since location is also captured with Site)

Note that we haven't yet checked for correlation between variables! We'll do that after we try to run a full model using vif() which also checks for multicollinearity.
...
```{r}
# Try to run the first model
lap_first_mod <- glmer(Bitter ~ Shell.Condition + (1 | Site) + (1 | s.Year),
                       data = allcrabs_dat,
                       family = binomial,
                       na.action = "na.fail",              # This line is for the dredge() function used later
                       control = glmerControl(optimizer = "bobyqa")   # This is to improve optimization
                       )

# Update the model to include the effect of temperature
lap_next_mod <- update(lap_first_mod, Bitter ~ Shell.Condition + s.Temp + (1 | Site) + (1 | s.Year))

# Update the model to include the effect of CW
lap_third_mod <- update(lap_next_mod, Bitter ~ Shell.Condition + s.Temp + s.CW + (1 | Site) + (1 | s.Year))

# Update the model to include the effect of black mat
lap_fourth_mod <- update(lap_third_mod, Bitter ~ Shell.Condition + s.Temp + s.CW + Blackmat + (1 | Site) + (1 | s.Year))

# Update the model to include the effect of leg condition
lap_fifth_mod <- update(lap_fourth_mod, Bitter ~ Shell.Condition + s.Temp + s.CW + Blackmat + Leg.Condition + (1 | Site) + (1 | s.Year))

# Update the model to include the effect of Julian day
lap_sixth_mod <- update(lap_fifth_mod, Bitter ~ Shell.Condition + s.Temp + s.CW + Blackmat + Leg.Condition + s.Jul.Day + (1 | Site) + (1 | s.Year))

# Update the model to include the effect of sex
lap_seven_mod <- update(lap_sixth_mod, Bitter ~ Shell.Condition + s.Temp + s.CW + Blackmat + Leg.Condition + s.Jul.Day + Sex + (1 | Site) + (1 | s.Year))

# Update the model to include the effect of depth
lap_eight_mod <- update(lap_seven_mod, Bitter ~ Shell.Condition + s.Temp + s.CW + Blackmat + Leg.Condition + s.Jul.Day + Sex + s.Depth + (1 | Site) + (1 | s.Year))

# Update the model to include the effect of latitude
lap_nine_mod <- update(lap_eight_mod, Bitter ~ Shell.Condition + s.Temp + s.CW + Blackmat + Leg.Condition + s.Jul.Day + Sex + s.Depth + s.Latitude + (1 | Site) + (1 | s.Year))

# Update the model to include the effect of longitude
lap_ten_mod <- update(lap_nine_mod, Bitter ~ Shell.Condition + s.Temp + s.CW + Blackmat + Leg.Condition + s.Jul.Day + Sex + s.Depth + s.Latitude + s.Longitude + (1 | Site) + (1 | s.Year))

# Oh my god, we actually got everything in our model!!! Incredible - it just straight-up broke the first time when I tried with even putting in two variables simultaneously. Go us!
